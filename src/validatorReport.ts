...
const ERADEPTH = (<any>data).ERADEPTH;
...
async function validatorReport() {
  ...
  const dotValidators = VALIDATORS.filter(
    (validator) => validator.network === "polkadot"
  );
  ...
  let dataset: dataset[] = await Promise.all(
    [
      dotValidators.map((validator) =>
        getAPIData(validator, "DOT", ERADEPTH, activeEraDOT, apiDOT)
      ),
      ksmValidators.map((validator) =>
        getAPIData(validator, "KSM", ERADEPTH, activeEraKSM, apiKSM)
      ),
    ].flat()
  );
  const tvpData = {
...
//Limiting typing for using the API interface
interface dataset {
...
async function getAPIData(
  ...
  let promiseLedger = api.query.staking
    ...
    .then((controller) =>
      api.query.staking.ledger<Option<StakingLedger>>(controller.toString())
    );
...
function printSummary(dataset: dataset[], tvpData) {
  ...
  dataset.forEach((data) => {
    ...
    if (tvpData[data["validator"].network]) {
      tvpIdx = tvpData[data["validator"].network].findIndex(
    ...
    data["erasStakers"].forEach((erasStakers, era) => {
      ...
      let eraPoints = getEraRewardPoints(
        data["validator"].stash,
        data["erasRewardPoints"][era]
      );
      process.stdout.write(Number(eraPoints).toLocaleString("en-US"));

      process.stdout.write(" (Average was ");
      let eraValidatorCount = new BN(
        data["erasRewardPoints"][era].individual.size
      );
...
function printNominators(dataset: dataset[]) {
...
function getPlanck(network: string): BN {
  if (network === "polkadot" || network === "DOT") return new BN(10 ** 10);
  else if (network === "kusama" || network === "KSM") return new BN(10 ** 12);
